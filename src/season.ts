...
export function seasonReport(month: number, lat: number, lng: number): SeasonReport {
  ...
  if (season === "spring") {
    ...
    items.push(
      ...
      { icon: "⚠︎", label: "Watch", note: "Heat and dehydration; afternoon thunderstorms and flash floods in the West." },
    );
  } else if (season === "fall") {
    items.push(
    ...
    items.push(
      ...
      { icon: "⚠︎", label: "Watch", note: "Cold is the killer — hypothermia, short days, and thin ice." },
    );
  }
