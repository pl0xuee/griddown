...
import { loadElevationGrid as loadGrid, tileXf, tileYf, pixelAt } from "./demgrid";

export type { ElevationGrid, DemTileSource } from "./demgrid";
...
/** Bulk-sample the ACTIVE DEM root over a bbox. See demgrid.ts for the why. */
export function loadElevationGrid(
  bounds: { west: number; south: number; east: number; north: number },
  z = 12
) {
  // Pin the root now: setDemRoot may fire while we await.
  return loadGrid(current, bounds, z);
}
